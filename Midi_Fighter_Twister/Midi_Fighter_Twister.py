#
# ./Ableton MIDI Remote Scripts/Midi_Fighter_Twister/,
#
...
class Midi_Fighter_Twister(ControlSurface):
    ...
    def _setup_session_control(self):
        ...
        self._session.set_track_bank_buttons(self._shift_bank_map[SESSIONRIGHT], self._shift_bank_map[SESSIONLEFT])
    ...
    def _setup_mixer_control(self):
        ...
        self._mixer.master_strip().set_volume_control(self._shift_encoder_map[MASTERVOLUME])
 ...
 # have right side btns act as device next/prev & toggle on/off?
    ...
    def _setup_device_and_transport_control(self):
        ...
        self._device = BestBankDeviceComponent(device_selection_follows_track_selection=True)
        ...
        for index in range(4): # 1st row, shift encoder hold
            device_param_controls.append(self._shift_encoder_map[PARAMCONTROL[index]])
        for index in range(6): # 2nd row & left-half of 3rd row, CC hold
            device_param_controls.append(self._note_map[PARAMCONTROL[index + 4]])
        ...
        detail_view_toggler.set_device_clip_toggle_button(self._shift_bank_map[CLIPTRACKVIEW])
        detail_view_toggler.set_detail_toggle_button(self._shift_bank_map[DETAILVIEW])
        # detail_view_toggler.set_device_nav_buttons(self._note_map[DEVICENAVLEFT], self._note_map[DEVICENAVRIGHT] )
        detail_view_toggler.set_device_nav_buttons(self._shift_bank_map[DEVICENAVLEFT], self._shift_bank_map[DEVICENAVRIGHT] )
    ...
    def _load_MIDI_map(self):
        ...
        for note in range(128):
            ...
            shift_bank_button = ButtonElement(is_momentary, MESSAGETYPE, SHIFTBANKCHANNEL, note)
            shift_bank_button.name = 'Note_' + str(note)
            self._shift_bank_map.append(shift_bank_button) # Shift Bank
            shift_encoder = ButtonElement(is_momentary, 1, ENCODERSHIFTCHANNEL, note)
